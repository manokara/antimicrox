...
void importLegacySettingsIfExist() {
    ...
    if (legacy.exists() && legacy.isFile()) {
        ...
        if (!current.exists()) {
            if (QFile::copy(PadderCommon::configLegacyFilePath(),
                            PadderCommon::configFilePath())) {
                qDebug() << "Legacy antimicroX settings copied";
                QMessageBox msgBox;
                msgBox.setText(
                    "Your original settings (previously stored in "
                    "~/.config/antimicroX) have been copied to "
                    "~/.config/antimicrox to "
                    "ensure consistent naming across entire project, if you "
                    "want you "
                    "can delete original directory or leave it as it is");
                msgBox.exec();
            } else {
                qDebug() << "Problem with importing antimicroX settings from: "
                ...
                msgBox.setText(
                    ...
                    "configs "
                    "are stored in ~/.config/antimicroX, but their new "
                    "location is "
                    "~/.config/antimicrox\nYou can do it manually by renaming "
                    "old "
                    "directory and renaming file antimicroX_settings.ini to "
...
int main(int argc, char *argv[]) {
    ...
    qRegisterMetaType<JoyButtonSlot::JoySlotInputAction>(
    ...
    parser.setApplicationDescription(QCoreApplication::translate(
        "antimicrox",
        "Graphical program used to map keyboard buttons and mouse controls to "
        "a "
    ...
    parser.addOptions({
        ...
        {"tray", QCoreApplication::translate(
                     "main", "Launch program in system tray only.")},
        // A boolean option with multiple names (-f, --force)
        {"no-tray", QCoreApplication::translate(
                        "main", "Launch program with the tray menu disabled")},
        // An option with a value
        {"hidden",
         QCoreApplication::translate(
             "main", "Launch program without the main window displayed")},
        {"profile",
         QCoreApplication::translate(
             "main", "Launch program with the configuration file selected as "
                     "the default "
                     "for selected controllers. Defaults to all controllers"),
         QCoreApplication::translate("main", "location")},
        {"profile-controller",
         QCoreApplication::translate(
             "main", "Apply configuration file to a specific controller. Value "
        ...
        {"startSet",
         QCoreApplication::translate(
             "main", "Start joysticks on a specific set. Value "
                     "can be a controller index, name, or GUID"),
         QCoreApplication::translate("main", "number value")},
        {{"daemon", "d"},
         QCoreApplication::translate(
             "main", "Launch program as a daemon. Use only on Linux.")},
        ...
        {"eventgen",
         QCoreApplication::translate(
             ...
             "generation. Use only if you have enabled xtest and uinput "
             "options "
             "on Linux or vmulti on Windows. Default: xtest."),
         QCoreApplication::translate("main", "event-generation-type"),
         "xtest"}, // default
        {{"list", "l"},
         QCoreApplication::translate(
             "main", "Print information about joysticks detected by SDL. Use "
                     "only if you have sdl library. You can check your "
                     "controller index, name or even GUID.")},
        // {"display",
        ...
        //      mapping
        //      window of selected controller. Value can be a controller index
        //      or
        //      GUID."),
        //      QCoreApplication::translate("main", "value")},
    });
    ...
    parser.addOption(QCommandLineOption(
        "tray", QObject::trUtf8("Launch program in system tray only.")));
    parser.addOption(QCommandLineOption(
        "no-tray",
        QObject::trUtf8("Launch program with the tray menu disabled")));
    parser.addOption(QCommandLineOption(
        "hidden",
        QObject::trUtf8("Launch program without the main window displayed")));

    parser.addOption(QCommandLineOption(
        "profile",
        QObject::trUtf8(
            "Launch program with the configuration file selected as the "
            "default "
            "for selected controllers. Defaults to all controllers"),
    ...
    parser.addOption(QCommandLineOption(
        "unload", QObject::trUtf8("Unload currently enabled profile(s)"),
        QObject::trUtf8("value(s)")));
    ...
    parser.addOption(QCommandLineOption("log-level",
                                        QObject::trUtf8("Enable logging"),
                                        QObject::trUtf8("log-type")));

    parser.addOption(QCommandLineOption(
        "log-file", QObject::trUtf8("Choose a file for logs writing"),
        QObject::trUtf8("filename")));

    parser.addOption(QCommandLineOption(
        "eventgen",
        QObject::trUtf8(
            "Choose between using XTest support and uinput support for event "
            "generation. Use only if you have enabled xtest and uinput options "
            "on Linux or vmulti on Windows. Default: xtest."),
    ...
    parser.addOption(QCommandLineOption(
        QStringList() << "list"
                      << "l",
        QObject::trUtf8(
            "Print information about joysticks detected by SDL. Use "
            "only if you have sdl library. You can check your "
            "controller index, name or even GUID.")));
    ...
    } else if (cmdutility.getCurrentLogLevel() !=
               appLogger.getCurrentLogLevel()) {
    ...
    QMap<SDL_JoystickID, InputDevice *> *joysticks =
        new QMap<SDL_JoystickID, InputDevice *>();
    ...
    if ((socket.serverName() == QString())) {
        ...
        if (!socket.waitForConnected(3000)) {
            ...
            qDebug()
                ...
                << socket.error() << "\nerror text 2 ->" << socket.errorString()
                << endl;
        } else {
    ...
    if (socket.state() == QLocalSocket::ConnectedState) {
        ...
        AntiMicroSettings settings(PadderCommon::configFilePath(),
                                   QSettings::IniFormat);

        // Update log info based on config values
        if (cmdutility.getCurrentLogLevel() == Logger::LOG_NONE &&
            settings.contains("LogLevel")) {
            appLogger.setLogLevel(static_cast<Logger::LogLevel>(
                settings.value("LogLevel").toInt()));
        }
        ...
        QPointer<InputDaemon> joypad_worker =
            new InputDaemon(joysticks, &settings, false);
        ...
        QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                         joypad_worker.data(), &InputDaemon::quit);
    ...
    if (cmdutility.launchAsDaemon()) {
        ...
        if (pid == 0) {
            ...
            appLogger.LogError(QObject::tr("Failed to launch daemon"), true,
                               true);
        ...
        if (QApplication::platformName() == QStringLiteral("xcb")) {
            ...
            if (cmdutility.getDisplayString().isEmpty()) {
                ...
                if (X11Extras::getInstance()->display() == nullptr) {
                    appLogger.LogError(
                        QObject::tr("Display string \"%1\" is not valid.")
                            .arg(cmdutility.getDisplayString()),
                        true, true);
        ...
        if (sid < 0) {
            appLogger.LogError(
                QObject::tr("Failed to set a signature id for the daemon"),
                true, true);
        ...
        if ((chdir("/")) < 0) {
            appLogger.LogError(
                QObject::tr("Failed to change working directory to /"), true,
                true);
    ...
    QStringList appDirsLocations =
        QStandardPaths::standardLocations(QStandardPaths::DataLocation);
    QStringList themePathsTries = QStringList();

    QList<QString>::const_iterator i;

    for (i = appDirsLocations.constBegin(); i != appDirsLocations.constEnd();
         ++i) {
    ...
    AntiMicroSettings *settings = new AntiMicroSettings(
        PadderCommon::configFilePath(), QSettings::IniFormat);
    ...
    if (cmdutility.getCurrentLogLevel() == Logger::LOG_NONE &&
        ...
        appLogger.setLogLevel(
            static_cast<Logger::LogLevel>(settings->value("LogLevel").toInt()));
    }
    ...
    if (cmdutility.shouldListControllers()) {
        ...
        QPointer<InputDaemon> joypad_worker =
            new InputDaemon(joysticks, settings);
        inputEventThread = new QThread;

        MainWindow *mainWindow =
            new MainWindow(joysticks, &cmdutility, settings);

        QObject::connect(&antimicrox, &QApplication::aboutToQuit, mainWindow,
                         &MainWindow::removeJoyTabs);
        QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                         joypad_worker.data(), &InputDaemon::quit);
        QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                         joypad_worker.data(), &InputDaemon::deleteJoysticks,
                         Qt::BlockingQueuedConnection);
        QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                         &PadderCommon::mouseHelperObj,
                         &MouseHelper::deleteDeskWid, Qt::DirectConnection);
        QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                         joypad_worker.data(), &InputDaemon::deleteLater,
                         Qt::BlockingQueuedConnection);
    ...
    EventHandlerFactory *factory =
        EventHandlerFactory::getInstance(cmdutility.getEventGenerator());
    ...
    if (!status && cmdutility.getEventGenerator() !=
    ...
    if (!status) {
        appLogger.LogError(
            QObject::tr("Failed to open event generator. Exiting."));
        appLogger.Log();
    ...
    QObject::connect(mainWindow, &MainWindow::joystickRefreshRequested,
                     joypad_worker.data(), &InputDaemon::refresh);
    QObject::connect(joypad_worker.data(), &InputDaemon::joystickRefreshed,
                     mainWindow, &MainWindow::fillButtonsID);
    QObject::connect(joypad_worker.data(), &InputDaemon::joysticksRefreshed,
                     mainWindow, &MainWindow::fillButtonsMap);

    QObject::connect(&antimicrox, &QApplication::aboutToQuit, localServer,
                     &LocalAntiMicroServer::close);
    QObject::connect(&antimicrox, &QApplication::aboutToQuit, mainWindow,
                     &MainWindow::saveAppConfig);
    QObject::connect(&antimicrox, &QApplication::aboutToQuit, mainWindow,
                     &MainWindow::removeJoyTabs);
    QObject::connect(&antimicrox, &QApplication::aboutToQuit, &mainAppHelper,
    ...
    QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                     joypad_worker.data(), &InputDaemon::deleteLater);
    QObject::connect(&antimicrox, &QApplication::aboutToQuit,
                     &PadderCommon::mouseHelperObj, &MouseHelper::deleteDeskWid,
                     Qt::DirectConnection);
    ...
    if (quitComboKeys != "") {
        ...
        QObject::connect(gs, &QGlobalShortcut::activated, &antimicrox,
                         &QApplication::quit);

        QObject::connect(&antimicrox, &QApplication::aboutToQuit, gs,
                         &QGlobalShortcut::deleteLater);
    }
#endif

    QObject::connect(localServer, &LocalAntiMicroServer::clientdisconnect,
                     mainWindow, &MainWindow::handleInstanceDisconnect);
